#ifndef MuonBasicDistributionsDataForBmmUrsAssociation_cxx
#define MuonBasicDistributionsDataForBmmUrsAssociation_cxx
...
#include "MuonBasicDistributionsDataForBmmUrsAssociation.h"
#include "../utilities/constants.h"



void MuonBasicDistributionsDataForBmmUrsAssociation::Loop()
{
  if (fChain == 0) return;
...
//   const char* genPhiLabel = "gen #phi";
  ...
  std::vector<TH1D*>* hSelMuonDeltaPtOverPtFromGenp = CreateVectorOf1DHistograms<TH1D>("hSelMuonDeltaPtOverPtFromGenp","|#Delta(p_{T})/p_{T}-1| of gen particle from closest matching selected muon", 100, 0., 1., "|#Delta(p_{T})/p_{T}|", nMuLabel,sBinningVariable,vLowBins);
  ...
  std::vector<TH1D*>* hTrackPlusDeltaPtOverPtFromGenp = CreateVectorOf1DHistograms<TH1D>("hTrackPlusDeltaPtOverPtFromGenp","|#Delta(p_{T})/p_{T}-1| of positive gen particle from closest matching track", 100, 0., 1., "|#Delta(p_{T})/p_{T}|", "N_{tracks}",sBinningVariable,vLowBins);
...
//               std::cout << "                Gen->Reco:\n";
//               std::cout << "                   iGenParticle = " << iGenParticle << std::endl;
...
//               std::cout << "                Reco->Gen:\n";
//               std::cout << "                   iGenParticle = " << iGenParticleWithDistance.first << std::endl;
//               std::cout << "                   iTrack       = " << iTrackWithDistance.first << std::endl;
...
//           }
    ...
    for(int iTrack = 0; iTrack < numberOfTrack; iTrack++)
    {
      if(Track_pt->at(iTrack) > 4 && fabs(Track_eta->at(iTrack))<2.4)
      {
        std::pair<short int, double> iGenParticleWithDistance = FindGenParticleNearestToTrack(iTrack);
        bool matchOk = false;
        for(unsigned int iGenParticleFromB = 0; iGenParticleFromB < vParticlesFromB.size(); iGenParticleFromB++)
        {
          if(iGenParticleWithDistance.first == vParticlesFromB[iGenParticleFromB])
          {
            matchOk = true;
            break;
          }
        ...
        int iGenParticle = iGenParticleWithDistance.first;
        double binningVariableValue = GetGenBinningVariablesValues(iGenParticle).at(binningVariableIndex);
        double dPtOverPt = fabs((Track_pt->at(iTrack) - Genp_pt->at(iGenParticleWithDistance.first)) / Track_pt->at(iTrack));
        
        Fill(hTrackDeltaRFromGenp,iGenParticleWithDistance.second,binningVariableValue,vLowBins);
        Fill(hTrackDeltaPtOverPtFromGenp,dPtOverPt,binningVariableValue,vLowBins);
        if(Genp_charge->at(iGenParticle) > 0)
        {
          Fill(hTrackPlusDeltaRFromGenp,iGenParticleWithDistance.second,binningVariableValue,vLowBins);
          Fill(hTrackPlusDeltaPtOverPtFromGenp,dPtOverPt,binningVariableValue,vLowBins);
        }
        else
        {
          Fill(hTrackMinusDeltaRFromGenp,iGenParticleWithDistance.second,binningVariableValue,vLowBins);
          Fill(hTrackMinusDeltaPtOverPtFromGenp,dPtOverPt,binningVariableValue,vLowBins);
        }
        
        if(iGenParticleWithDistance.second < 0.12 && dPtOverPt < 0.3)
...
//           unsigned short iGenParticle = vParticlesInAccFromB[iParticleFromB];
          std::pair<short int, double> iGenParticleWithDistance = FindGenParticleNearestToTrack(trackRef);
          std::pair<short int, double> iTrackWithDistance = FindTrackNearestToGenParticle(iGenParticle);
//           short int iTrack = iTrackWithDistance.first;
          short int iTrack = trackRef;
          
          
          if(iGenParticleWithDistance.first == iGenParticle  /*&& iTrackWithDistance.second < 0.1*/)
          {
...
std::string MuonBasicDistributionsDataForBmmUrsAssociation::GetBinningVariableString(const std::string sBinning)
{
...
std::vector<double> MuonBasicDistributionsDataForBmmUrsAssociation::GetBins(const std::string sBinning)
{
...
std::vector<double> MuonBasicDistributionsDataForBmmUrsAssociation::ParseBins(const std::string sBins)
{
...
std::vector<double> MuonBasicDistributionsDataForBmmUrsAssociation::GetBinningVariablesValues(const unsigned int iMuon)
{
...
std::vector<double> MuonBasicDistributionsDataForBmmUrsAssociation::GetGenBinningVariablesValues(const unsigned int iGenParticle)
{
  std::vector<double> result;
  result.push_back(1e100);
...
#endif // MuonBasicDistributionsDataForBmmUrsAssociation_cxx
